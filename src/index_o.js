  ...
  renderRefresh() {
    if (!this.props.state.dailyMode) {
      ...
      return (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="icon icon-tabler icon-tabler-refresh invisible"
          width="24"
          height="24"
          viewBox="0 0 24 24"
          strokeWidth="1.5"
...
class Settings extends React.Component {
  renderHardMode() {
    if (this.props.state.dailyMode) {
      return(
      <div className="settings-opcion hidden">
        <div>
              ...
              <div>
                <div className="onoffswitch">
                  <input
                    ...
                    id="myonoffswitch-1"
                    tabIndex={1}
                    checked={this.props.state.dailyMode}
                    onChange={() => this.props.cambiarModoDiario()}
                  />
                  <label
                    className="onoffswitch-label"
...
class App extends React.Component {
  ...
  cambiarModoDiario(){
    let nuevoModo = !this.state.dailyMode;
  ...
  getDailyWord() {
    if (this.state.dailyMode) {
      var randomNumber = new Date().toLocaleDateString();
      var test = new Date();
      console.log(words[Math.floor(test.getDay() % words.length)]);
      this.setState({
        dailyWord: this.encriptarPalabra(
          words[Math.floor(test.getDay() + (test.getMonth() % words.length))]
        ),
      });
    }
  }
    ...
    return (
      <div className="game">
        <div className="game-main">
          ...
          <Keyboard
            onClick={(i) => this.keyPress(i)}
            llenarArray={() => this.llenarArray()}
            guardarEstado={() => this.guardarEstado()}
          />
        </div>
        ...
        <div className="game-settings hidden scale-up-center">
          <Settings
            ...
            cambiarModoDiario={() => this.cambiarModoDiario()}
            state={this.state}
          />
        </div>
